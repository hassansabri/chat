...
function createChatBox(chatboxtitle, minimizeChatBox, user_name, newMsgContent, reciever_id, chat_type, message_type) {
    ...
    if (chat_type == 'group') {
        chatboxtitle = 'group' + chatboxtitle
    } else {
    ...
    if (chat_type != "group") {
        ...
        if (flag == "yes") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    var obj = JSON.parse(data);
                    $("#chatbox_group" + user_id + " .chatboxcontent").append(obj.html);
                    $("#chatbox_group" + user_id + " .chatboxcontent").scrollTop($("#chatbox_group" + user_id + " .chatboxcontent")[0].scrollHeight);
                }
            });
        } else {
            if (typeof message !== 'undefined' && $.trim(message) != "" && message != false) {
                if (message_type == "file") {
                    $("#chatbox_group" + user_id + " .chatboxcontent").append('<div class="chatboxmessage sender"><span class="chatboxmessagefrom sender">' + name + ':&nbsp;&nbsp;</span><span class="chatboxmessagecontent"><a target="_blank" href="' + global_url + 'attachments/org/' + message.new_name + '"><br/><img class="txtpng" src="' + global_url + 'assets/images/txt.png"/> &nbsp;&nbsp; ' + message.event.file.name + ' </a></span></div>');
                    $("#chatbox_group" + user_id + " .chatboxcontent").scrollTop($("#chatbox_group" + user_id + " .chatboxcontent")[0].scrollHeight);
                } else {
                    $("#chatbox_group" + user_id + " .chatboxcontent").append('<div class="chatboxmessage sender"><span class="chatboxmessagefrom sender">' + name + ':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">' + message + '</span></div>');
                    $("#chatbox_group" + user_id + " .chatboxcontent").scrollTop($("#chatbox_group" + user_id + " .chatboxcontent")[0].scrollHeight);
                }
